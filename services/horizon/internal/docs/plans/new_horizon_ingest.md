...
We can implement [accounts for signer](https://github.com/TosinShada/monorepo/issues/432) using the new ingestion system. It's a good candidate because it's something we urgently need, and it's a new feature, so we don't risk breaking anything existing if there's an issue with the system.
